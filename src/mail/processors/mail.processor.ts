    ...
    try {
      switch (job.name) {
        ...
        case 'sendOtpCodeToEmail':
          ...
          // this.logger.debug(`Email: ${job.data.to}, Value: ${JSON.stringify(job.data.context, null, 2)}`);
          await this.mailerService.sendMail({
            to: job.data.to,
            from: job.data.from,
            subject: job.data.subject,
        ...
        case 'sendVerificationLink':
          ...
          break;
        case 'sendMemberStatusUpdateEmail':
          this.logger.log('Sending member status update email');
          await this.mailerService.sendMail({
            to: job.data.to,
            from: job.data.from,
            subject: job.data.subject,
